...
TOOLS — You have direct access to these tools. This is a HARD RULE, not a
suggestion: for ANY question about current prices, news, trends, or competitor
activity, you MUST call the relevant tool BEFORE writing your answer. Do not
answer from memory and do not tell the user to search themselves — that defeats
the purpose of having these tools.

- `web_search`: call this for ANY question involving prices (gold, diamond, or
  anything else), news, current events, market trends, or anything that could
  have changed recently. Examples that REQUIRE calling web_search first:
  "harga emas hari ini berapa?", "berita jewelry terbaru apa?", "tren diamond
  2026 gimana?". If you're not 100% certain a fact is still true today, search
  first — do not guess and do not deflect the user to search it themselves.
- `get_social_media_intel`: call this for ANY question about competitors,
  social media trends, or what's viral/trending in the jewelry industry right now.

If a tool result doesn't fully answer the question, say so honestly rather than
guessing. Never fabricate prices, statistics, or current events — always use
web_search for anything you're not certain is still accurate today.

Other input formats the user may use (handled separately, just respond naturally
to the content given to you):
...
async def ask_claude(messages: list, extra_system: str = "", use_tools: bool = True) -> str:
    ...
    logging.info(f"ask_claude: use_tools={use_tools}, tools_attached={len(kwargs.get('tools', []))}, last_user_msg={messages[-1].get('content', '')[:100] if messages else 'N/A'}")

    for round_num in range(MAX_TOOL_ROUNDS):
        response = claude.messages.create(**kwargs)
        logging.info(f"ask_claude round {round_num}: stop_reason={response.stop_reason}, block_types={[b.type for b in response.content]}")
